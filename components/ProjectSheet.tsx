                ...
                {Array.from({ length: COLS }, (_, c) => {
                  ...
                  return (
                    <td
                      ...
                      className={`border border-border px-1.5 overflow-hidden whitespace-nowrap text-ellipsis relative ${inSel(r, c) && !isActive ? 'bg-brand-blue/5' : ''} ${isEditingHere ? 'outline outline-2 outline-brand-green -outline-offset-1 z-[6]' : isActive ? 'outline outline-2 outline-brand-blue -outline-offset-1 z-[5]' : ''} ${d.num ? 'text-right font-mono text-ink' : 'text-left text-ink'} ${d.err ? 'text-status-red font-bold' : ''} ${(cells[ref] || '').startsWith('=') && !d.err ? 'text-brand-blue' : ''}`}
                    >
