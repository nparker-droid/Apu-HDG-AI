    ...
    const chaptersWithTotals = chapters
      ...
      .map(chapter => {
        const chapterApus = apus
          ...
          .sort((a, b) => a.code.localeCompare(b.code, undefined, { numeric: true }))
          .map(apu => {
            // Lógica de cálculo idéntica a tus exportaciones
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
          ...
          <p className="text-3xl font-black text-[#004071]">{formatCurrency(budgetData.totalNetoProyecto)}</p>
        </div>
        ...
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
          ...
          <p className="text-3xl font-black text-slate-500">{formatCurrency(budgetData.totalNetoProyecto * 0.19)}</p>
        </div>
        ...
        <div className="bg-[#004071] p-6 rounded-[2rem] shadow-xl text-white">
          ...
          <p className="text-3xl font-black">{formatCurrency(budgetData.totalNetoProyecto * 1.19)}</p>
        </div>
      </div>
            ...
            {budgetData.chaptersWithTotals.map(chapter => (
              <React.Fragment key={chapter.id}>
                <tr className="bg-slate-50">
                  ...
                  <td colSpan={3} className="px-8 py-3 font-black text-[#004071] text-xs uppercase">{chapter.name}</td>
                  <td className="px-8 py-3 text-right font-black text-[#004071] text-xs">{formatCurrency(chapter.totalChapter)}</td>
                </tr>
                {chapter.apus.map(apu => (
                  <tr key={apu.id} className="border-b border-slate-50 hover:bg-slate-50/30 transition-colors">
                    ...
                    <td className="px-8 py-3 text-[10px] text-center text-slate-400">{apu.unit}</td>
                    <td className="px-8 py-3 text-[10px] text-center text-slate-400">{apu.quantity}</td>
                    <td className="px-8 py-3 text-xs text-right font-bold text-slate-700">{formatCurrency(apu.subtotal)}</td>
                  </tr>
                ))}
              </React.Fragment>
            ))}
