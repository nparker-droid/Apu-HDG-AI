...
const LibraryModal: React.FC<LibraryModalProps> = ({ onClose, onSelect, projectApus }) => {
...
const LibraryItem: React.FC<LibraryItemProps> = ({ apu, onSelect, isProject }) => (
  <div 
  ...
  >
    <div className="flex flex-col gap-1 overflow-hidden pr-4">
      <div className="flex items-center gap-2">
        <span className={`text-[8px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest ${isProject ? 'bg-[#88C13E]/10 text-[#88C13E] group-hover:bg-white/20 group-hover:text-white transition-colors' : 'bg-slate-100 text-slate-500 group-hover:bg-white/10 group-hover:text-white transition-colors'}`}>
          {isProject ? 'Referencia Proyecto' : `Referencia: ${apu.code || 'STD'}`}
        </span>
      </div>
